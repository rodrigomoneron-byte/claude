# Instruções de Loop — [TÍTULO DA SÉRIE], Livro [N] (Claude Code)
...
3. **Validar régua (trava dura)**:
   `python3 scripts/regua_gate.py outputs/conteudo/livroN-capituloNN.md`
...
- A cada fim de ato (definir os números no onboarding, com base na
  estrutura da bíblia): mini-auditoria — alternância de POV, cadência de
  heat até aqui, símbolos costurados, aderência à bíblia. Corrigir
...
2. **Formatação** (skill bia-ferreira-formatter): organizar manuscrito
   ...
   series_index=[N], título travado no onboarding), EPUB + PDF + prompt
   de capa (usar references/branding-rbg.md).

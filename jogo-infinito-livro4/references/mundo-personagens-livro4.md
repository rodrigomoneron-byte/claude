# Mundo e Personagens — Jogo Infinito, Livro 4: Legado e Reinvenção

## Estado herdado no fechamento do Livro 3
- Cami e Alex: casal estabelecido há mais de dois anos, moram juntos,
  não são casados (decisão explícita mantida — ver seção "Regras
  fixas"). Cami confirma gravidez no cap. 60 do L3 (teste positivo,
  exame de sangue marcado pra "amanhã"); ainda não contaram pra
  ninguém além um do outro no fechamento do L3.
- Sebastian Kroll: reconciliado, Meridian Holdings reestruturada em
  escala menor (3 mercados, não mais 12), em terapia, reconstruindo
  relação com a própria família. Mantém contato profissional
  ocasional, cordial, como colega de mercado — NÃO like antagonista
  disponível de novo neste livro (arco fechado com dignidade no L3;
  reabrir seria contradizer o próprio fechamento).
- Devon Ashworth: promovida a Diretoria de Inteligência Estratégica
  (cap. 48/54 do L3), reportando direto à liderança.
- Marcus Boone (COO) e Naomi: filha (nome ainda não decidido no L3,
  cogitado "Sofia") nascendo por volta do fechamento do L3/abertura
  do L4 — checar timeline exata ao abrir o L4 (gravidez estava em
  estágio avançado no cap. 59 do L3).
- Elena Voss: noiva de Marco (professor de história da arte),
  casamento ainda sem data fechada no L3 ("provavelmente ano que
  vem").
- Denise: assistente executiva, cargo consolidado.
- Priya: analista freelancer contratada como reforço temporário
  (cap. 27-28 do L3) — pode continuar como funcionária efetiva ou
  encerrar contrato, a definir na escrita.
- Conselho: David Okonkwo, Sandra Whitmore, Thomas Reyes (Thomas
  processou reconciliação de perspectiva ao longo do L3, hoje aliado
  ativo).
- Dan Carter: procurador, amigo, nunca POV — herdado sem mudança.
- Alexandra Voss Whitfield (prima, L2): gancho aberto ("reconstruir
  com integridade real, algum dia") permanece NÃO resolvido neste
  livro — reservado pra L5, salvo decisão explícita em contrário
  durante a escrita.
- Símbolos ativos: relógio de bolso parado do pai da Cami (verificação
  > pressa), caneta de Alex, pequena réplica do relógio dada por Alex
  no cemitério (cap. 51 do L3) — três objetos já na estante no
  fechamento do L3, com espaço simbólico para um "quarto companheiro"
  (o bebê) mencionado explicitamente na última cena.

## Elenco novo deste livro
- **Antagonista/concorrente externo** (nome a definir na escrita,
  primeiro capítulo em diante) — fundo ou consórcio rival competindo
  pela MESMA vantagem que a gravidez da Cami torna urgente: um
  processo de sucessão/governança na Voss Capital. Ver arco completo
  em arco-trama-livro4.md. Regra herdada: sem vilão caricato, motivação
  compreensível, nenhuma reaproveitação de nome/fato de Gelo e Sangue
  ou de qualquer outro livro da casa.
- Personagem de apoio novo possível: um candidato interno ou externo à
  sucessão que o conselho considera, criando tensão genuína sem ser
  traição (ninguém do elenco herdado atua de má-fé) — a definir na
  escrita.

## Linha do tempo
Livro 4 abre poucas semanas a poucos meses depois do fechamento do L3
— tempo suficiente pra Cami confirmar a gravidez, o casal decidir quem
mais avisar, e a pressão de sucessão começar a se formar organicamente
(não é notícia da gravidez que gera a crise de sucessão sozinha — é a
combinação de gravidez + primeiro movimento real de um concorrente de
olho na Voss Capital em transição).
...
- SEM traição interna neste livro (default assumido — ver
  project_dna.md seção 8) — qualquer atrito no conselho ou na equipe é
  desacordo estratégico de boa-fé.
- Casamento: não travado como evento deste livro — pode ser mencionado
  como conversa/planejamento, mas o casamento oficial fica disponível
  pro L5 se fizer sentido, evitando empilhar dois eventos-marco (bebê
